...
public class BitmapUtil {
     ...
     * @return
     */
    public static Bitmap stringToBitmap(String str64) {
        Bitmap bitmap = null;
        try {
            byte[] bitmapArray;
    ...
    public static String saveBitmap(Bitmap bitmap, String name) {
        // 先判断目录是否存在
        try {
            File dir = new File(getFilePath(name));
    ...
    public static String uri2Path(final Context context, Uri uri) {
        if (null == uri)
            return null;
    ...
    public static Bitmap getBitmapFromUri(Context ctx, Uri uri) {
        ...
        try {
            ...
            outputStream.flush();
            outputStream.close();
            return bytes2Bimap(contentPicByte);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    // radius = 0;
    // if (DEBUG) {
    // LogUtil.w(TAG, "radius must be 0 < r < 25 , forcing radius=0");
    // }
    // } else if (radius > 25) {
    // radius = 25;
    // if (DEBUG) {
    // LogUtil.w(TAG, "radius must be 0 < r < 25 , forcing radius=25");
    // }
    // }
    //
